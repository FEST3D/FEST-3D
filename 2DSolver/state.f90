module state
    ...
    contains
       ...
       !    x_speed(:, jmx) = x_speed(:, jmx - 1)
       !    y_speed(:, jmx) = y_speed(:, jmx - 1)
       !    ! If the flow is subsonic, impose the back pressure
       !    ! Else, impose the inlet pressure
       !    ! Extrapolate at the other end
       !    if (supersonic_flag .eqv. .TRUE.) then
       ...
       !    end if
        ...
        subroutine set_top_and_bottom_ghost_cell_data()
         ...
         !  pressure(0, 1:jmx-1) = pressure(1, 1:jmx-1)
         !  pressure(imx, 1:jmx-1) = pressure(imx-1, 1:jmx-1)
         !  density(0, 1:jmx-1) = density(1, 1:jmx-1)
         !  density(imx, 1:jmx-1) = density(imx-1, 1:jmx-1)
        ...
        subroutine apply_flow_tangency_conditions()
            ...
            if (mu_ref /= 0.0) then
                ...
                call dmsg(1, 'state', 'apply_flow_tangency_conditions', 'No Slip BC')
    ...
    !       x_speed(imx, 1:jmx-1) = x_speed(imx-1, 1:jmx-1) - &
    !               (2. * &
    !                   ((x_speed(imx-1, 1:jmx-1) * &
    !                       xnx(imx, 1:jmx-1) * xnx(imx, 1:jmx-1)) &
    !                   + (y_speed(imx-1, 1:jmx-1) * &
    !                       xny(imx, 1:jmx-1) * xnx(imx, 1:jmx-1)) &
    !                   ) &
    !               )
    !       y_speed(imx, 1:jmx-1) = y_speed(imx-1, 1:jmx-1) - &
    !               (2. * &
    !                   ((x_speed(imx-1, 1:jmx-1) * &
    !                       xnx(imx, 1:jmx-1) * xny(imx, 1:jmx-1)) &
    !                   + (y_speed(imx-1, 1:jmx-1) * &
    !                       xny(imx, 1:jmx-1) * xny(imx, 1:jmx-1)) &
    !                   ) &
    !               )
    !       
    !       ! For the bottom cells
    !       x_speed(0, 1:jmx-1) = x_speed(1, 1:jmx-1) - &
    !               (2. * &
    !                   ((x_speed(1, 1:jmx-1) * &
    !                       xnx(1, 1:jmx-1) * xnx(1, 1:jmx-1)) &
    !                  + (y_speed(1, 1:jmx-1) * &
    !                       xny(1, 1:jmx-1) * xnx(1, 1:jmx-1)) &
    !                   ) &
    !               )
    !       y_speed(0, 1:jmx-1) = y_speed(1, 1:jmx-1) - &
    !               (2. * &
    !                   ((x_speed(1, 1:jmx-1) * &
    !                       xnx(1, 1:jmx-1) * xny(1, 1:jmx-1)) &
    !                  + (y_speed(1, 1:jmx-1) * &
    !                       xny(1, 1:jmx-1) * xny(1, 1:jmx-1)) &
    !                   ) &
    !               )
