...
```RunDir = 'FlatPlate_Lam'```   Name of the run directory to create for current case. <br>
...
```Scheme['TurbulenceModel']='none'``` since no turbulence is required, __none__ is mentioned here. <br>
```Scheme['TransitionModel']='none'``` Similar to turbulence model option, __none__ is metioned for no transition model. <br>
...
```Flow["UInf"] = 173.59``` Free-stream x-component of velcotiy vector. (Mach = 0.5 at T_ref=300K)<br> 
...
```Flow["ReferenceViscosity"] = 2.083e-4``` Reference viscosity is set to 0.0002083 kg/ms so that the Reynolds number (L_ref=1m) is 1 million<br>
...
```BoundaryConditions = [-8, -4, -5, -6, -6, -6]``` Broad boundary condition.[Riemann inlet, subsonic outlet, no-slip wall and slip wall for rest ]. Since the bottom faces
...
Rest of the variables should be left to their default value. In order to execute this script, use the following command:
```
$python automaton.py
```
Now you will see a new folder created with ```RunDir``` name (_FlatPlate_Lam_). Switch to that directory to run the test case. To make sure setup is correct, check
...
```
Change the Jmin boundary condition for first two blocks from no-slip wall (-5) to slip wall (-6).
```
## BLOCK LAYOUT FILE
## ==========================
## NUMBER OF PROCESSES
...
<figure>
  <div style="display: flex; flex-wrap: wrap; justify-content: center; align-items:center">
    <img src="|media|/tutorials/LaminarFlatPlateCf.png" alt="Contour of v speed" style="width:350px">
  </div>
  <div style="display: flex; flex-wrap: wrap; justify-content: center; align-items:center">
    <figcaption> Fig.4 Coefficient of friction along the surface of the plate. </figcaption>
  </div>
</figure>
