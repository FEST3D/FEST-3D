...
```Flow["DensityInf"] = 1.17659``` Free-stream density.<br>
```Flow["UInf"] = 69.445``` Free-stream x-component of velcotiy vector. (Mach = 0.5 at T_ref=300K)<br> 
...
```Flow["ReferenceViscosity"] = 1.63416586e-5``` Reference viscosity is set such that the Reynolds number (L_ref=1m) is 5 million<br>
