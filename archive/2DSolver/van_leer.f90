module van_leer
    ...
    contains
        ...
        !   F_plus(imx, 1:jmx-1, 1) = 0
        !   F_plus(1, 1:jmx-1, 1) = 0
        ...
        !   F_minus(imx, 1:jmx-1, 1) = 0
        !   F_minus(1, 1:jmx-1, 1) = 0
        ...
        subroutine compute_fluxes()
            ...
            F = compute_F_plus() + compute_F_minus()
            if (any(isnan(F))) then
                call dmsg(5, 'van_leer', 'compute_flux', 'ERROR: F flux Nan detected')
                stop
            end if

            G = compute_G_plus() + compute_G_minus()
            if (any(isnan(G))) then
                call dmsg(5, 'van_leer', 'compute_flux', 'ERROR: F flux Nan detected')
                stop
            end if
