...
latest_fvtk=$(ls output*.fvtk | sort -V | tail -n 1)
mv $latest_fvtk $backup_dir/
rm *.fvtk
...
cp plotandsavelogs.py $backup_dir/

cd $backup_dir/
echo 'Saving plots of the log files...' | tee -a $runlog
python plotandsavelogs.py
cd ..
