module HLU_SGS
  ...
  use global_vars, only: x_qp_left
  use global_vars, only: y_qp_left
  use global_vars, only: z_qp_left
  use global_vars, only: x_qp_right
  use global_vars, only: y_qp_right
  use global_vars, only: z_qp_right
...
!      end select
    ...
    subroutine DiagonalBlock(fdir)
      implicit none
      integer, intent(in) :: fdir
      integer :: i,j,k
      real :: DivV, phi, a1, a2
      real :: gm_minus_one
      real :: r,u,v,w,p,E
      ...
      select case (f_dir)
        ...
        case default
          DebugCall("DiagonalBlock:Direction error")
          stop
      end select
      ...
      do k = 1, kmx - 1 + k_f
        do j = 1, jmx - 1 + j_f 
          do i = 1, imx - 1 + i_f
            gm_minus_one = gm - 1.0
            r = 0.5*(f_qp_left(i,j,k,1) + f_qp_right(i,j,k,1))
            u = 0.5*(f_qp_left(i,j,k,2) + f_qp_right(i,j,k,2))
            v = 0.5*(f_qp_left(i,j,k,3) + f_qp_right(i,j,k,3))
            w = 0.5*(f_qp_left(i,j,k,4) + f_qp_right(i,j,k,4))
            p = 0.5*(f_qp_left(i,j,k,5) + f_qp_right(i,j,k,5))
            E = (p/(gm_minus_one) + 0.5*(V2/r)
            V2 = u*u + v*v + w*w
            Vn = u*nx + v*ny + w*nz
            phi = 0.5*gm_minus_one*V2
            a1  = gm*E - phi
            a2  = gm_minus_one
            D(1,1) = 0.0
            D(1,2) = nx
        ...
        do k=1,kmx-1
          do j = 1,jmx-1
            do i = 1,imx-1
              ...
              qp(i,j,k,5:n_var) = max(qp(i,j,k,5:n_var), 1.e-10)
            end do
          end do
        end do
...
!      Flux(5) = Flux(5) - ( Tauxy * uface + Tauyy * vface + Tauyz * wface + Qy ) * ny
!      Flux(5) = Flux(5) - ( Tauxz * uface + Tauyz * vface + Tauzz * wface + Qz ) * nz
!      Flux(6) = Flux(6) + (mmu + sigma_k1*tmu)*(dtkdx*nx + dtkdy*ny + dtkdz*nz)
!      Flux(7) = Flux(7) + (mmu + sigma_w1*tmu)*(dtwdx*nx + dtwdy*ny + dtwdz*nz)
!
!      Flux    = Flux * Area
!      SSTFlux = Flux
!
