            ...
            do k = 1, kmx - 1
             do j = 1, jmx - 1
              do i = 1, imx - 1
               ...
               lmx1 = abs( &
                    (0.5*(x_x_speed_left(i, j, k)+x_x_speed_right(i, j, k)) * xnx(i, j, k)) + &
                    (0.5*(x_y_speed_left(i, j, k)+x_y_speed_right(i, j, k)) * xny(i, j, k)) + &
                    (0.5*(x_z_speed_left(i, j, k)+x_z_speed_right(i, j, k)) * xnz(i, j, k))) + &
                    x_sound_speed_avg
               ! For front face, i.e., lower index face along eta direction
               lmx2 = abs( &
                    (0.5*(y_x_speed_left(i, j, k)+y_x_speed_right(i, j, k)) * ynx(i, j, k)) + &
                    (0.5*(y_y_speed_left(i, j, k)+y_y_speed_right(i, j, k)) * yny(i, j, k)) + &
                    (0.5*(y_z_speed_left(i, j, k)+y_z_speed_right(i, j, k)) * ynz(i, j, k))) + &
                    y_sound_speed_avg
               ! For bottom face, i.e., lower index face along zeta direction
               lmx3 = abs( &
                    (0.5*(z_x_speed_left(i, j, k)+z_x_speed_right(i, j, k)) * znx(i, j, k)) + &
                    (0.5*(z_y_speed_left(i, j, k)+z_y_speed_right(i, j, k)) * zny(i, j, k)) + &
                    (0.5*(z_z_speed_left(i, j, k)+z_z_speed_right(i, j, k)) * znz(i, j, k))) + &
                    z_sound_speed_avg
               ...
               lmx4 = abs( &
                    (0.5*(x_x_speed_left(i+1, j, k)+x_x_speed_right(i+1, j, k)) * xnx(i+1, j, k)) + &
                    (0.5*(x_y_speed_left(i+1, j, k)+x_y_speed_right(i+1, j, k)) * xny(i+1, j, k)) + &
                    (0.5*(x_z_speed_left(i+1, j, k)+x_z_speed_right(i+1, j, k)) * xnz(i+1, j, k))) + &
                    x_sound_speed_avg
               ! For back face, i.e., higher index face along eta direction
               lmx5 = abs( &
                    (0.5*(y_x_speed_left(i, j+1, k)+y_x_speed_right(i, j+1, k)) * ynx(i, j+1, k)) + &
                    (0.5*(y_y_speed_left(i, j+1, k)+y_y_speed_right(i, j+1, k)) * yny(i, j+1, k)) + &
                    (0.5*(y_z_speed_left(i, j+1, k)+y_z_speed_right(i, j+1, k)) * ynz(i, j+1, k))) + &
                    y_sound_speed_avg
               ! For top face, i.e., higher index face along zeta direction
               lmx6 = abs( &
                    (0.5*(z_x_speed_left(i, j, k+1)+z_x_speed_right(i, j, k+1)) * znx(i, j, k+1)) + &
                    (0.5*(z_y_speed_left(i, j, k+1)+z_y_speed_right(i, j, k+1)) * zny(i, j, k+1)) + &
                    (0.5*(z_z_speed_left(i, j, k+1)+z_z_speed_right(i, j, k+1)) * znz(i, j, k+1))) + &
                    z_sound_speed_avg
