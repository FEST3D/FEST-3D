...
#include "mpi.inc"
  ...
  real, dimension(:,:,:,:), allocatable :: delQk12
  ...
  public :: update_with_SGSR
  public :: setup_SGSR
  public :: destroy_SGSR

  contains

    subroutine setup_SGSR()
      implicit none
      character(len=*), parameter :: &
        errmsg="module: SGSR, subrouinte setup"
    ...
    end subroutine setup_SGSR

    subroutine destroy_SGSR()
      implicit none
      ...
      call dealloc(dummy)
    end subroutine destroy_SGSR

    subroutine update_with_SGSR()
      implicit none
    ...
    end subroutine update_with_SGSR
    ...
    subroutine update_laminar_variables()
      ...
      integer :: i,j,k, SubIter
        ...
        do k=1,kmx-1
          do j=1,jmx-1
            do i=1,imx-1
              ...
              delQk12(i,j,k,1:5) = &
                      ((DelIminusFlux - LambdaTimesArea(1)*delQstar(i-1,j,k,1:5)) &
                     + (DelJminusFlux - LambdaTimesArea(2)*delQstar(i,j-1,k,1:5)) &
                     + (DelKminusFlux - LambdaTimesArea(3)*delQstar(i,j,k-1,1:5)) )
            end do
          end do
        ...
        delQ=0.0
        !call apply_interface(delQk12, 1)
        !backward sweep
            do i=imx-1,1,-1
          do j=jmx-1,1,-1
        do k=kmx-1,1,-1
              ...
              delQ(i,j,k,1:5) = (-residue(i,j,k,1:5) &
                - 0.5*((DelIminusFlux - LambdaTimesArea(4)*delQ(i+1,j,k,1:5)) &
                     ...
                     + (DelKminusFlux - LambdaTimesArea(6)*delQ(i,j,k+1,1:5)) &
                     + delQk12(i,j,k,1:5)))/D

              delQk12(i,j,k,1:5) = &
                      ((DelIminusFlux - LambdaTimesArea(4)*delQ(i+1,j,k,1:5)) &
                     + (DelJminusFlux - LambdaTimesArea(5)*delQ(i,j+1,k,1:5)) &
                     + (DelKminusFlux - LambdaTimesArea(6)*delQ(i,j,k+1,1:5)) )
            end do
          end do
        ...
        do k=1,kmx-1
          do j=1,jmx-1
            do i=1,imx-1
              ...
              delQk12(i,j,k,1:7) = &
                      ((DelIminusFlux - LambdaTimesArea(1)*delQstar(i-1,j,k,1:7)) &
                     + (DelJminusFlux - LambdaTimesArea(2)*delQstar(i,j-1,k,1:7)) &
                     + (DelKminusFlux - LambdaTimesArea(3)*delQstar(i,j,k-1,1:7)) )
            end do
          end do
        ...
        do k=kmx-1,1,-1
              ...
              delQ(i,j,k,1:7) = (-residue(i,j,k,1:7) &
                - 0.5*((DelIminusFlux - LambdaTimesArea(4)*delQ(i+1,j,k,1:7)) &
                     ...
                     + (DelKminusFlux - LambdaTimesArea(6)*delQ(i,j,k+1,1:7)) &
                     + delQk12(i,j,k,1:7)))/D

              delQk12(i,j,k,1:7) = &
                      ((DelIminusFlux - LambdaTimesArea(4)*delQ(i+1,j,k,1:7)) &
                     + (DelJminusFlux - LambdaTimesArea(5)*delQ(i,j+1,k,1:7)) &
                     + (DelKminusFlux - LambdaTimesArea(6)*delQ(i,j,k+1,1:7)) )
          ...
          do j=1,jmx-1
            do i=1,imx-1
              ...
              delQk12(i,j,k,1:n_var) = &
                      ((DelIminusFlux - LambdaTimesArea(1)*delQstar(i-1,j,k,1:n_var)) &
