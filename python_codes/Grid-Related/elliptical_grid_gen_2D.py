...
def get_one_directional_stretching(x, n):
    # See page 2: http://www3.nd.edu/~gtryggva/CFD-Course/2011-Lecture-25.pdf
    a = x[0]
    b = x[-1]
#   n = 1.8
    x_norm = (x-a)/(b-a)
    x_new = a + (x_norm ** (n))*(b - a)
    return x_new
...
def one_directional_stretching_matrix(Y):
    ...
    for i in range(p):
        ...
        y_norm = (y-a)/(b-a)
        y_new = a + (y_norm ** (n))*(b - a)
     #  y_new = a + ((1 + (np.tanh(delta*(y_norm-1.))/np.tanh(delta/1.0))))*(b-a)
        Y[i, :] = y_new
...
#                   round((y_limits[1] - y_limits[0]) / dy + 1), endpoint=True)
    ...
    x = np.linspace(x_limits[0], x_limits[1], 101)
  # x1 = np.linspace(0, -0.06, 50)
  # x1 = get_one_directional_stretching(x1, 1.8).tolist()
  # x1.reverse()
  # x1 = np.array(x1)
  # x2 = np.linspace(0, 1.2192, 251)
  # x2 = get_one_directional_stretching(x2, 1.8)
  # x = np.append(x1[:-1], x2)
                    
    y = np.linspace(y_limits[0], y_limits[1], 51)
...
def get_starting_grid(bump='bottom'):   
    ...
    Y = one_directional_stretching_matrix(Y)
...
def main_function():
    ...
    gridfile = 'bumpgrid-smooth-3.txt'
    datafile = None
    opfilename = 'bumpgrid-smooth-3.vtk'
    filecomment = 'Testing 2D grid for Blasius Flow'
    bump = 'bottom'
